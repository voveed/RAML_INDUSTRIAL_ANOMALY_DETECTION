...
# ========================================================================================
# WINCLIP REPRODUCTION - PHASE 2 (KAGGLE VERSION): MAIN MODEL RECOVERY + ABLATION
# ========================================================================================
# HƯỚNG DẪN SỬ DỤNG TRÊN KAGGLE:
# 1. Tạo Notebook mới.
# 2. Add Dataset 'mvtec-anomaly-detection' (hoặc tương tự) vào input.
# 3. Bật GPU P100/T4.
# 4. Copy toàn bộ code này vào chạy.
# ========================================================================================
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("🔬 [PHASE 2 - KAGGLE] RUNNING ABLATION STUDIES")
    print("="*60)
    
    ablation_results = {}
    runs = [
        # Main RAML is running in Tab 1, skipping it here.
        # Variants ONLY (Total ~4 hours)
        {'name': 'Fixed Margin (0.5)',   'epochs': 20, 'lr': 5e-5, 'margin_base': 0.5, 'lambda_sigma': 0.0, 'lambda_resolution': 0.0},
        {'name': 'Strong Margin (1.0)',  'epochs': 20, 'lr': 5e-5, 'margin_base': 1.0, 'lambda_sigma': 0.0, 'lambda_resolution': 0.0}, 
        {'name': 'Standard BCE',         'epochs': 20, 'lr': 5e-5, 'loss': 'bce'},
    ]
    
    # Using 'train_ratio=0.8' to match experimental setup
    ...
    # Define path on Kaggle for output
    results_path = 'winclip_phase2_ablation_results.json'
    
    for run in runs:
        print(f"\n▶️ Running Ablation: {run['name']}")
        
        # Fresh model for each run
        model = RAMLUltimateModel(clip_model).to(device)
        model = train_ablation_run(model, train_loader, run, device)
        score = evaluate_run(model, test_loader, device)
        
        # Save Result Immediately (Score)
        ablation_results[run['name']] = score
        print(f"✅ Result {run['name']}: {score*100:.2f}%")
        
        # Save Model Checkpoint
        clean_name = run['name'].replace(" ", "_").replace("(", "").replace(")", "").replace(".", "")
        ckpt_path = f"model_{clean_name}.pt"
        torch.save(model.state_dict(), ckpt_path)
        print(f"💾 Saved Model to: {ckpt_path}")
        
        # Write Results to local
        with open('ablation_results.json', 'w') as f: json.dump(ablation_results, f, indent=4)
