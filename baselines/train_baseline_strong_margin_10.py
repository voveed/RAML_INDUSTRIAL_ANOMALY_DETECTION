...
class AutoDifficultyTracker:
    ...
    def get_weight(self, cat):
        ...
        if cat not in self.running_loss: return 1.0
...
class TextVisualFusion(nn.Module):
    ...
    def forward(self, visual_features: torch.Tensor, text_features: torch.Tensor) -> torch.Tensor:
        ...
        if text_proj.dim() == 2: text_proj = text_proj.unsqueeze(0).expand(B, -1, -1)
...
class RAMLUltimateModel(nn.Module):
    ...
    def encode_text_prompts(self, device, categories):
        ...
        with torch.no_grad():
            ...
            for cat in categories:
                if cat in self._text_cache: batch_emb.append(self._text_cache[cat])
                else:
    ...
    def forward(self, images, categories=None):
        ...
        text_emb = self.encode_text_prompts(images.device, categories)
    ...
    def __init__(self, data_dir, categories, split='train', transform=None, use_real_anomalies=False, train_ratio=0.8):
        ...
        for category in categories:
            ...
            if split == 'train':
                ...
                if use_real_anomalies: self.samples.extend(test_bad[:split_b])
            else:
                self.samples.extend(test_good[split_g:])
...
def train_ablation_run(model, train_loader, args, device):
    ...
    for epoch in range(1, args['epochs'] + 1):
        ...
        for imgs, labels, cats in pbar:
            ...
            # Metric Learning (MACCL) + BCE Head
...
def evaluate_run(model, test_loader, device):
    ...
    for cat, data in results_by_cat.items():
        lbls, scrs = zip(*data)
        if len(set(lbls)) > 1: auroc_by_cat[cat] = roc_auc_score(lbls, scrs)
        else: auroc_by_cat[cat] = 0.5
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("🔬 [PHASE 2 - KAGGLE] RUNNING BASELINE: STRONG MARGIN 1.0")
    print("="*60)
    
    run = {'name': 'Strong Margin (1.0)', 'epochs': 20, 'lr': 5e-5, 'margin_base': 1.0, 'lambda_sigma': 0.0, 'lambda_resolution': 0.0}
    ...
    print(f"✅ Final Result Strong Margin (1.0): {score*100:.2f}%")
    os.makedirs("../checkpoints", exist_ok=True)
    torch.save(model.state_dict(), "../checkpoints/model_Strong_Margin_10.pt")
    print("💾 Saved checkpoint to ../checkpoints/model_Strong_Margin_10.pt")
    ...
    with open("../results/result_Strong_Margin_10.json", "w") as f:
        json.dump(result_data, f, indent=4)
    print("💾 Saved results to ../results/result_Strong_Margin_10.json")
